...
# @st.cache_resource (1.18 이후 버전)
@st.cache(allow_output_mutation=True) # decorator: data caching
def load_data():
...
r_year = st.selectbox("추천 연도 선택", options=[None] + sorted(ratings_df['rating_year'].unique()))
r_month = st.selectbox("추천 월 선택", options=[None] + sorted(ratings_df['rating_month'].unique()))
